...
class EmployeServices {
 ...
 static Future<List<EmployeeDto>> getEmployesNotAssignedToCurrentProject(String serverIp) async {
  ...
  static Future<EmployeeDto> updateEmployee (String serverIp, EmployeeDto employeeDto) async {
  ...
  static Future<void> deleteEmployee(String serverIp, int id) async {
    final response = await http.delete(Uri.parse('http://$serverIp:8080/api/v1/employe/$id'));
    if (response.statusCode != 200) {
      throw Exception('Failed to delete employe');
    }
  }

  static Future<List<DeviceDto>> getDevices(String serverIp, int employeeId) async {
    final response = await http.get(Uri.parse('http://$serverIp:8080/api/v1/employe/device/$employeeId'));
    if (response.statusCode == 200) {
      final jsonResponse = convert.jsonDecode(response.body);
      final responseDto = ResponseDto.fromJson(jsonResponse);
      final List<dynamic> dataList = responseDto.data;
      final List<DeviceDto> devices = dataList.map((e) => DeviceDto.fromJson(e)).toList();
      return devices;
    } else {
  ...
  static Future<bool> postDevice(String serverIp, int employeeId, String deviceMac) async {
    final response = await http.post(
